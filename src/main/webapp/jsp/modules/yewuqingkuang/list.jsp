...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">业务情况管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active"><span>业务情况列表</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">业务情况列表</h3>							
							<div class="table-responsive mb-3" id="tableDiv">
								<div class="col-sm-12">
									<label>
										事项名称
									</label>
									<input type="text" id="shixiangmingchengSearch" class="form-control form-control-sm" placeholder="请输入事项名称" aria-controls="tableId">
									<label>
										类型
									</label>
								...
								<table id="tableId" class="table table-bordered table-striped">
									<thead>
										<tr>
											...
											<th onclick="sort('shixiangmingcheng')">事项名称<i id="shixiangmingchengIcon" class="fa fa-sort"></i></th>
											<th onclick="sort('leixing')">类型<i id="leixingIcon" class="fa fa-sort"></i></th>
											<th>代办内容</th>
											<th onclick="sort('daibanfeiyong')">代办费用<i id="daibanfeiyongIcon" class="fa fa-sort"></i></th>
											...
											<th onclick="sort('shifouwancheng')">是否完成<i id="shifouwanchengIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
			if($('#shixiangmingchengSearch').val() != null && $('#shixiangmingchengSearch').val() != ''){
				searchForm.shixiangmingcheng ="%" +  $('#shixiangmingchengSearch').val() + "%";
			}
        ...
        function getDataList() {
			http("yewuqingkuang/page","GET",{
				...
				sort: sortColumn,
				order: sortOrder,
				shixiangmingcheng : searchForm.shixiangmingcheng,	
		...
		function setDataRow(item,number){
			...
			var shixiangmingchengCell  = document.createElement('td');

						 shixiangmingchengCell.innerHTML = item.shixiangmingcheng;

			row.appendChild(shixiangmingchengCell);
			var leixingCell  = document.createElement('td');

						 leixingCell.innerHTML = item.leixing;

			row.appendChild(leixingCell);
			var daibanneirongCell  = document.createElement('td');

						  if(item.daibanneirong != null && item.daibanneirong != "" && item.daibanneirong.length >= 11){
			  	daibanneirongCell.innerHTML = item.daibanneirong.substring(0, 10) + "..."; 
			  }else{
			  	daibanneirongCell.innerHTML = item.daibanneirong;
			  }


			row.appendChild(daibanneirongCell);
			var daibanfeiyongCell  = document.createElement('td');

						 daibanfeiyongCell.innerHTML = item.daibanfeiyong;

			row.appendChild(daibanfeiyongCell);
			...
			var yewuqingkuangCell  = document.createElement('td');

			
			var shifouwanchengCell  = document.createElement('td');

						  shifouwanchengCell.innerHTML = item.shifouwancheng; 

			row.appendChild(shifouwanchengCell);
		...
		function sh(){
			...
			httpJson("yewuqingkuang/update","POST",data,(res)=>{
				if(res.code == 0){
					alert("提交成功");
					getDataList();
				}
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("yewuqingkuang/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
	...
	function crossTable(obj,crossTableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
		...
		window.sessionStorage.setItem('crossTableName',"yewuqingkuang");
        ...
        function pay(item){
			...
			window.sessionStorage.setItem('tableName',"yewuqingkuang");
