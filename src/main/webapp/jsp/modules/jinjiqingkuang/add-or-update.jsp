...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑紧急情况</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>紧急情况管理</span></li>
								<li class="breadcrumb-item active"><span>编辑紧急情况</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">紧急情况信息</h3>
									...
									<div class="form-group col-md-6">
										...
										<div id="shijianDate" class="input-append date form_datetime">
											...
											<div class="form-group  col-md-12">
												<label>详细</label>
												<div><div class="upload"><input id="xiangxiupload" name="file" type="file">选择文件</div>
												<script id="xiangxiEditor" type="text/plain"></script>
												<script type="text/javascript">
												    //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
												   //相见文档配置属于你自己的编译器
												 var xiangxiUe = UE.getEditor('xiangxiEditor', {
													toolbars: [
														[
															'undo', //撤销
															'bold', //加粗
												...
												<input type="hidden" name="xiangxi" id="xiangxi-input">
												</div>
											</div>	
	...
	<script>
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "jinjiqingkuang/info/" + id,
		...
		function upload(){
			...
			$('#xiangxiupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('xiangxiEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
				...
				if(crossrefid && crossuserid) {
					...
					http("jinjiqingkuang/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("jinjiqingkuang/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		// 填充富文本框
		function setContent(){
			if(ruleForm.xiangxi != null && ruleForm.xiangxi != 'null' && ruleForm.xiangxi != ''){
				var xiangxiMes = '' + ruleForm.xiangxi;
				var xiangxiUeditor = UE.getEditor('xiangxiEditor');
				xiangxiUeditor.ready(function() {
					xiangxiUeditor.setContent(xiangxiMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('xiangxiEditor').hasContents()){
				$('#xiangxi-input').attr('value',UE.getEditor('xiangxiEditor').getPlainTxt());
			}
		}
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("jinjiqingkuang/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function calculationSE(colName){
			//单列求和接口
			http("jinjiqingkuang"+colName,"GET",{
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.xiangxi != null && res.data.xiangxi != ''){

						$("#xiangxi").val(res.data.xiangxi);
						$("#xiangxi").attr('readonly','readonly');
					}
