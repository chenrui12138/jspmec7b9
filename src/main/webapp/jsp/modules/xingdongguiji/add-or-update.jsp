...
<body>
		...
		<div id="contentText">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">编辑行动轨迹</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item"><span>行动轨迹管理</span></li>
								<li class="breadcrumb-item active"><span>编辑行动轨迹</span></li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">行动轨迹信息</h3>
							<form id="addOrUpdateForm">
								<div class="form-row">
									...
									<div class="form-group col-md-6">
										<label>住址</label>
										<textarea  name="zhuzhi" class="form-control" id="zhuzhi" rows="3"></textarea>
									</div>
									<div class="form-group col-md-6">
										<label>提交时间</label>
									...
									<div class="form-group col-md-6">
										...
										<textarea  name="beizhu" class="form-control" id="beizhu" rows="3"></textarea>
											...
											<div class="form-group  col-md-12">
												<label>轨迹描述</label>
												<div><div class="upload"><input id="guijimiaoshuupload" name="file" type="file">选择文件</div>
												<script id="guijimiaoshuEditor" type="text/plain"></script>
												<script type="text/javascript">
												    //建议使用工厂方法getEditor创建和引用编辑器实例，如果在某个闭包下引用该编辑器，直接调用UE.getEditor('editor')就能拿到相关的实例
												   //相见文档配置属于你自己的编译器
												 var guijimiaoshuUe = UE.getEditor('guijimiaoshuEditor', {
													toolbars: [
														[
															'undo', //撤销
															'bold', //加粗
												...
												<input type="hidden" name="guijimiaoshu" id="guijimiaoshu-input">
												</div>
											</div>	
	...
	<script>
		  ...
		  beforeCreate: function(){
			  var id = window.sessionStorage.getItem("id");
				if(id != null && id != "" && id != "null"){
					$.ajax({ 
		                type: "GET",
		                url: baseUrl + "xingdongguiji/info/" + id,
		...
		// 文件上传
		function upload(){
			$('#guijimiaoshuupload').fileupload({
				url: baseUrl + 'file/upload',
				headers:{ token: window.sessionStorage.getItem("token")},
				dataType: 'json',
				done: function (e, data) {	
					UE.getEditor('guijimiaoshuEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
				}
			});
		}  
		//取消 
				...
				if(crossrefid && crossuserid) {
					...
					http("xingdongguiji/page","GET",{
						...
						if(res.data.total >= crossoptnum){
							...
							httpJson("xingdongguiji/"+urlParam,"POST",data,(res)=>{
								if(res.code == 0){
		...
		// 填充富文本框
		function setContent(){
			if(ruleForm.guijimiaoshu != null && ruleForm.guijimiaoshu != 'null' && ruleForm.guijimiaoshu != ''){
				var guijimiaoshuMes = '' + ruleForm.guijimiaoshu;
				var guijimiaoshuUeditor = UE.getEditor('guijimiaoshuEditor');
				guijimiaoshuUeditor.ready(function() {
					guijimiaoshuUeditor.setContent(guijimiaoshuMes);
				});
			}
		}  
		...
		// 获取富文本框内容
		function getContent(){
			if(UE.getEditor('guijimiaoshuEditor').hasContents()){
				$('#guijimiaoshu-input').attr('value',UE.getEditor('guijimiaoshuEditor').getPlainTxt());
			}
		}
		...
		function getDetails() {
			...
			if(id != null && id != "" && id != "null"){
				updateId = id;
				window.sessionStorage.removeItem('id');
				http("xingdongguiji/info/" + id,"GET",{},(res)=>{
					if(res.code == 0){
						ruleForm = res.data
						// 是/否下拉框回显
						setSelectOption();
		...
		function calculationSE(colName){
			//单列求和接口
			http("xingdongguiji"+colName,"GET",{
			...
			function fill(){
				...
				http(accountTableName+'/session','GET',{},(res)=>{
					if(res.code == 0){
						...
						if(res.data !=null && res.data.zhuzhi != null && res.data.zhuzhi != ''){
		...
		function dataBind(){
			...
			$("#guijimiaoshu").val(ruleForm.guijimiaoshu);	
		...
		if(crossTableName != null && crossTableName != '' && crossTableId != null && crossTableId != '' && crossTableId != -1){
			http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
				if(res.code == 0){
					...
					if(res.data != null && res.data != '' && res.data.guijimiaoshu != null && res.data.guijimiaoshu != ''){

						$("#guijimiaoshu").val(res.data.guijimiaoshu);
						$("#guijimiaoshu").attr('readonly','readonly');
					}
